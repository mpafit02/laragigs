<x-layout>
    <x-card class="p-10 max-w-lg mx-auto mt-24">
        <header class="text-center">
            ...
            <p class="mb-4">Edit: {{$listing->title}}r</p>
        </header>

        {{-- We need to define that this is a POST method --}}
        {{-- We need to define enctype="multipart/form-data" because we have file upload --}}
        <form method="POST" action="/listings/{{$listing->id}}" enctype="multipart/form-data">
            {{-- we need the @csrf to avoid attacks --}}
            @csrf 

            {{-- We need to make this a PUT method  --}}
            @method('PUT')
            <div class="mb-6">
